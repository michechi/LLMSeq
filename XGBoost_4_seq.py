...
if __name__ == "__main__":
    ...
    if args.tokenizer == "Base":
        ...
        if args.first_time:
            ...
            np.save(f"{args.embedding_dir}X_train_{args.model_name}_embeddings.npy", X_train_encoded)
            ...
            np.save(f"{args.embedding_dir}X_val_{args.model_name}_embeddings.npy", X_val_encoded)
            ...
            np.save(f"{args.embedding_dir}X_test_{args.model_name}_embeddings.npy", X_test_encoded)
            logger.info(f"Test embeddings shape: {X_test_encoded.shape}")
        else:
            # If not first time, let's re-load embeddings
            X_train_encoded = np.load(f"{args.embedding_dir}X_train_{args.model_name}_embeddings.npy")
            X_val_encoded = np.load(f"{args.embedding_dir}X_val_{args.model_name}_embeddings.npy")
            X_test_encoded = np.load(f"{args.embedding_dir}X_test_{args.model_name}_embeddings.npy")
