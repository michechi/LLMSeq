...
def efficient_check_v1(sequences, c_vocab, assign_outcome_positional, second_key, tolerance=True, lags=7, rnd=False, debugging=False):
    ...
    for seq in tqdm(sequences):
        df_results = pd.DataFrame(assign_outcome_positional(seq, c_vocab, second_key, already_splitted=False, lags=lags, tolerance=tolerance, rnd=rnd))
...
def process_single_sequence(args):
    ...
    seq, c_vocab, assign_outcome_positional, second_key, lags, tolerance, rnd, debugging, min_chain_length = args
    result = assign_outcome_positional(
        seq, c_vocab, second_key,
...
def efficient_check_parallel(sequences, c_vocab, assign_outcome_positional, second_key,
    ...
    args_list = [
        (seq, c_vocab, assign_outcome_positional, second_key, lags, tolerance, rnd, debugging, min_chain_length) 
