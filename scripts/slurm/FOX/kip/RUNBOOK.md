...
| optional | `kip_h_llama_m6.slurm` | Llama-3.2-1B LoRA m6 ordered, seeds 9550–9552 | 0–2 | 8 h | unknown (completes per-model m6 coverage) — **needs HF token**, see below |
...
cd $HOME/MIMICIV
git fetch origin
git checkout refactor/restructure-repo   # or Rebuttals_NeurIPS -- both carry the full KIP package (identical bytes)
git pull --ff-only origin refactor/restructure-repo
```
...
## 7b. Optional: Llama arms (kip_g, kip_h)

kip_g completes the Llama m4 triplet (seed 9550 runs locally as Block D);
kip_h adds the Llama m6 triplet (seeds 9550–9552), completing per-model m6
coverage. `meta-llama/Llama-3.2-1B` is **gated** — these are the only KIP
jobs that need your HF credentials. Either have a cached `huggingface-cli
login` (the scripts fall back to `~/.cache/huggingface/token`) or export the
token in the shell you submit from; it is never echoed or logged:

```bash
cd $HOME/MIMICIV/scripts/slurm/FOX/kip
export HF_TOKEN=<your token>        # skip if huggingface-cli login was done
sbatch kip_g_llama_m4.slurm
sbatch kip_h_llama_m6.slurm
```
...
If Llama m6 converges, its modes (b)/(c) run via
`sbatch --export=ALL,KIP_MODEL=Llama1B kip_f_m6_controls.slurm` (also needs
the token; checkpoint-consistent with kip_h).
...
git pull --rebase origin $(git branch --show-current)   # CSV has merge=union: local+fox rows both survive
...
Prerequisite: `git pull` on the branch carrying this package — `Rebuttals_NeurIPS`, or `refactor/restructure-repo` (full KIP pipeline ported 2026-09-14) (this round's slurm
...
sbatch kip_r_qwen27b_qlora.slurm       # 6 cells / 3 tasks — Qwen3.8-27B QLoRA (4-bit NF4, paper big-decoder recipe; ungated; pins transformers 5.14.1 user-site; controls later via KIP_MODE=)
```
