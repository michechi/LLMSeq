"""Step 5: build Dataset A (strict order-only cancer trajectory task)."""
...
logger = setup_logging("05_make_order_task")
...
def main() -> int:
    ...
    tables = build_dataset_a(notes)
    labels = tables["labels"]

    if labels.empty:
        logger.warning("no patients qualify for strict Dataset A")
    else:
        ...
        # Persist the patient-level split mapping (for join with Dataset B later).
        splits_path = paths.splits_dir / "patient_splits_order.parquet"
        split_df.to_parquet(splits_path)
        logger.info("wrote %d order-task splits -> %s", len(split_df), splits_path)

    out_dir = paths.dataset_order_only_dir
