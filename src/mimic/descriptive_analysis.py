"""
Descriptive analysis for ICML paper: lag analysis (natural λ) and
discriminative code identification (empirical S) for CKD→ESRD cohort.
"""
...
CCS_CAT = ROOT / "codes" / "CCS_DX_categories.csv"
...
# ── 1a. Lag Analysis ──────────────────────────────────────────────────
def lag_analysis(df):
    """For Y=1 progressors, compute spacing between kidney-related codes."""
    ...
    for _, row in progressors.iterrows():
        ...
        for j in range(1, len(positions)):
            lag = positions[j] - positions[j - 1] - 1  # intervening codes
            records.append({
...
def report_lag(lag_df):
    lags = lag_df["lag"]
    n_patients = lag_df["subject_id"].nunique()
    lines = [
        "=" * 60,
        "1a. LAG ANALYSIS  (natural λ)",
        "=" * 60,
...
# ── 1b. Discriminative Code Identification ────────────────────────────
def discriminative_codes(df, desc_map):
    """Compute per-code frequency ratio between Y=1 and Y=0."""
    y1 = df[df["label"] == 1]
    y0 = df[df["label"] == 0]
    n1, n0 = len(y1), len(y0)

    # Binary presence and counts per patient
    ...
    disc_df = pd.DataFrame(rows).sort_values("log_ratio", ascending=False, na_position="last")
    disc_df = disc_df.reset_index(drop=True)
    return disc_df


def report_disc(disc_df):
    lines = [
        "=" * 60,
    ...
    # Verify CCS_158 is NOT at top
    ckd_row = disc_df[disc_df["code"] == "CCS_158"].iloc[0]
    lines.append(f"Sanity check — CCS_158 (CKD) rank: "
                 f"log2R={ckd_row['log_ratio']:.2f}, "
                 f"freqY1={ckd_row['freq_Y1']:.3f}, "
                 f"freqY0={ckd_row['freq_Y0']:.3f}")
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    print(disc_report)
    disc_df.to_csv(OUT_DIR / "discriminative_codes.csv", index=False)

    # Full report
    full_report = header + "\n" + lag_report + "\n" + disc_report
    (OUT_DIR / "descriptive_stats.txt").write_text(full_report)
