...
class CausalLMWithClassificationHead(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, outcome_labels=None):
        ...
        causal_outputs = self.backbone(
            ...
            output_hidden_states=True, # False for memory efficiency
            return_dict=True
        )
...
def load_model_causal(model_name, model_type, tokenizer, cache_dir, hf_token, use_peft, use_quantization, tiny):
    """
    ...
    if not tiny:
        ...
        dimensions = {
            ...
            '10M':[512, 6, 8, 1024, 2000], # TO TEST
            '25M':[384, 6, 12, 1024, 4000], # TO TEST
