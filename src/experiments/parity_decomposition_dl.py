...
def _anchor_paths(data_dir: Path, ell: int, k: int) -> dict:
...
def load_variant_data(
    ...
    if anchor is not None:
        ell, k = anchor
        paths = _anchor_paths(data_dir, ell, k)
        logger.info("loading binary anchor ell=%d k=%d from %s", ell, k, data_dir)
    else:
...
def run(args: argparse.Namespace) -> dict:
    ...
    anchor: tuple[int, int] | None = None
    if args.size.startswith("binary"):
        # binary2 or binary4 → ell, k = default k = max(1, ell//2)
    ...
    run_key = _run_key(args.variant, args.model, args.size, args.seed, anchor=anchor)
