...
def _anchor_paths(data_dir: Path, ell: int, k: int) -> dict[str, Path]:
...
# Driver                                                                      #
# --------------------------------------------------------------------------- #
def _run_key(variant: str, size_tag: str, seed: int, anchor: tuple[int, int] | None) -> str:
    if anchor is not None:
        ell, k = anchor
        return f"anchor_l{ell}_k{k}_Llama32-1B_{seed}"
    return f"{variant}_Llama32-1B_{size_tag}_{seed}"
