...
def train_and_evaluate_causal(model, train_loader, val_loader, args, device, use_quantization=False):
    """Training loop with early stopping (no model checkpoint saving)."""
    ...
    for epoch in range(args.epochs):
        ...
        # Early stopping check (track metrics only, no model saving)
        condition = False
        if args.early == "auc":
    ...
    return best_auc, best_f1, best_val_loss, epoch + 1, best_val_preds, best_val_labels
