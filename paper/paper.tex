...
Recent empirical evidence suggests that the latter may often be true. \citet{kimLostSequenceLarge2025b} showed that LLM-based recommenders exhibit minimal performance degradation when item sequences are shuffled, indicating that these models do not effectively leverage sequential information. \citet{bediFidelityMedicalReasoning2025} found similar reliance on statistical pattern matching rather than genuine temporal reasoning in clinical settings. \citet{nguyen2024understanding} demonstrated that transformer predictions can often be approximated by simple k-gram rules (a k-gram is a contiguous subsequence of $k$ consecutive symbols, see~\ref{subsec:kgram}). These findings raise a critical concern: apparent sequential understanding may actually reflect the exploitation of local statistical shortcuts \cite{mccoy2019right, mirzadeh2024gsm}.
...
\begin{definition}
    For lag parameter $\lambda \geq 1$, we say $X$ is \emph{compliant} if there exists a chain of positions $(t_1, \ldots, t_k)$ with $k \geq 2$ such that: \textbf{(i)} $t_{j+1} - t_j = \lambda$ for all $j$. \textbf{(ii)} $X_{t_j} \in \mathcal{S}$ for all $j$, and \textbf{(iii)} $\kappa(X_{t_1}) \leq \cdots \leq \kappa(X_{t_k})$.
...
Labels are assigned probabilistically. Let $Y^{*} \in \{0,1\}$ denote the compliance indicator (i.e., if $X$ is compliant with respect to an ordering $\kappa$), then the associated compliance indicator $Y^{*}=1$. Now, let us denote with \textit{Y} the \textit{observed} outcome in the data. Then we have:
\begin{equation}
    \mathbb{P}(Y=1 \mid Y^*) = 
        \begin{cases} 
        1 - \pi & \text{if } Y^* = 1 \\
...
A binary classification problem exhibits \emph{pure sequential dependence} if and only if the outcome depends exclusively on the sequential ordering of elements, with no dependence on:
\textbf{(a)} individual element frequencies, \textbf{(b)} element co-occurrence patterns (order-independent), \textbf{(c)} aggregate sequence statistics, or \textbf{(d)} any order-invariant sequence properties.
\end{definition}

A natural expectation is that individual symbols should carry no information about the label. Under uniform sampling, $\mathbb{P}(X_t = a) = 1/|\mathcal{A}|$ for all $a$ and $t$, so one might expect $I(Y; X_t) = 0$, where
\begin{equation}
\label{eq:mutual_info}
    I(Y; X_t) =\sum_{y, a} \mathbb{P}(Y{=}y, X_t{=}a) \log \frac{\mathbb{P}(Y{=}y, X_t{=}a)}{\mathbb{P}(Y{=}y)\,\mathbb{P}(X_t{=}a)}
\end{equation}
...
Let $\mathcal{A}$ be a finite alphabet with $|\mathcal{A}| = \ell \geq 2$, and let $\kappa$ be a total order on $\mathcal{A}$. Let $X = (X_1, \ldots, X_n)$ be sampled i.i.d.\ uniformly from $\mathcal{A}$, and define the label $Y^{*} = \mathbf{1}\{X \text{ is sequentially compliant with } \kappa\}$. Then there exist a position $t$ and a symbol $a \in \mathcal{A}$ such that
\[
\mathbb{P}(X_t = a \mid Y^{*} = 1) \neq \mathbb{P}(X_t = a),
\]
and consequently $I(Y^{*}; X_t) > 0$ by equation~\ref{eq:mutual_info}.
\end{theorem}

\begin{proof}[Proof outline]
Let $\mathcal{C}_n \subset \mathcal{A}^n$ denote the set of compliant sequences. The number of non-decreasing sequences of length $n$ over $\ell$ symbols equals the number of ways to place $n$ identical items into $\ell$ bins, a classic combinatorial identity (sometimes called ``stars-and-bars'' \cite{Stars_Bars}) yielding $|\mathcal{C}_n| = \binom{n+\ell-1}{\ell-1}$. Fix $t=1$ and let $a_j$ have rank $j = \kappa(a_j)$. If $X_1 = a_j$, the remaining $n-1$ symbols must form a non-decreasing sequence over $\{a_j, \ldots, a_\ell\}$, giving $|\mathcal{C}_{n,1,j}| = \binom{n-1+\ell-j}{\ell-j}$ such sequences. Thus
\[
\mathbb{P}(X_1 = a_j \mid Y^{*}=1) = \frac{\binom{n-1+\ell-j}{\ell-j}}{\binom{n+\ell-1}{\ell-1}},
\]
which depends on $j$, while $\mathbb{P}(X_1 = a_j) = 1/\ell$ for all $j$. Hence $I(Y^{*}; X_1) > 0$. See Appendix~\ref{appendix:proof_impossibility} for the complete proof.
\end{proof}

The implication is fundamental: classification tasks based on event-level ordering \emph{cannot} satisfy pure sequential dependence. The ordering constraint creates statistical regularities detectable from local positions, explaining why even localized statistics such as k-grams carry discriminative information. Consider an alphabet $\{\texttt{A}, \texttt{B}, \ldots, \texttt{Z}\}$ and suppose we want sequences in non-decreasing order. If the first symbol is \texttt{A}, all 26 letters remain available for the rest of the sequence, making compliance relatively easy (and likely). If the first symbol is \texttt{Z}, only \texttt{Z} can follow, making compliance nearly impossible for long sequences. Thus, observing \texttt{Z} in position 1 strongly predicts non-compliance, even though, in the generation process, each letter appears with equal marginal probability $1/26$. This result extends to infinite sequences ($n=\infty)$ by restriction to finite prefixes.


\subsection{Operation-Based Sequentiality via Parity}
\label{sec:parity}

Theorem~\ref{thm:impossibility} establishes that ordering-based tasks inevitably leak local information. To test whether models can perform genuine global integration or merely exploit local shortcuts we need a task where no such shortcuts exist. We introduce an alternative task based on a sequence of algebraic operations that resemble what we called pure sequential dependence.

The intuition is simple: designate a subset of letters as ``key'' (set $\mathcal{S}$) and count how many times each key letter appears in the entire sequence. For each key letter, determine whether its count is even or odd. The label depends on the parity of how many key letters have an even count. Crucially, changing any single letter in the sequence can flip the label, so the model cannot rely on local patterns it must track counts across the entire sequence.


\begin{definition}[Parity Task]
\label{def:set-theoretic-parity}
Let $\mathcal{A}$ be a finite alphabet and $\mathcal{S} \subset \mathcal{A}$ a non-empty proper subset. For $X = (X_1, \ldots, X_n)$ with $X_t \overset{\text{i.i.d.}}{\sim} \text{Uniform}(\mathcal{A})$, let $N_s = \sum_{t=1}^{n} \mathbf{1}_{\{X_t = s\}}$ for each $s \in \mathcal{S}$, and define
\[
Y^{*} := \left| \{ s \in \mathcal{S} : N_s \text{ is even} \} \right| \mod 2.
\]
\end{definition}

\begin{theorem}[Local Information of Parity Task]
\label{prop:local-info-parity}
Let $p = |\mathcal{S}|/|\mathcal{A}|$.
\begin{itemize}
    \item If $p = 1/2$, then $I(Y^{*}; X_t) = 0$ for all $t$ and $n$.
    \item If $p \neq 1/2$, then $I(Y^{*}; X_t) > 0$ for finite $n$, but $I(Y^{*}; X_t) \to 0$ as $n \to \infty$.
\end{itemize}
\end{theorem}

\begin{proof}[Proof outline]
The core intuition is that the variable $Y^{*}$, despite being defined via individual counts $N_s$, is algebraically equivalent (modulo 2) to the parity of the \textit{total} count of elements falling in $\mathcal{S}$. Specifically, we show in the full proof (Appendix~\ref{app:proof_prop_local_info}) that $Y^{*}$ is a bijective transformation of the standard parity variable $T := (\sum_{t=1}^n \mathbf{1}_{\{X_t \in \mathcal{S}\}}) \mod 2$. 
Since mutual information is invariant under bijection, the vanishing mutual information of $T$ directly implies the result for $Y$.
\end{proof}
...
We instantiate three task variants with different properties, see Appendix~\ref{appendix:Figures} for an example of each different mechanisms.
...
\]
For $n=20$, $\ell=26$: $\rho_{\mathrm{naive}} \approx 10^{-16}$ (see Appendix~\ref{proof:naive_setting}), making the task degenerate without probabilistic labeling. Due to this extremely low compliance probability, we construct the naive dataset first generating alphabetically ordered (compliant) sequences and then creating non-compliant counterparts by permuting selected elements within these sequences.


\paragraph{Tricky setting} ($m \ll \ell$, $\lambda > 1$, $\kappa = \text{random but fixed}$).
Only a small subset of letters matters, relevant letters must appear at specific lag intervals, and the ordering key is randomized. For $m=6$, $\lambda=7$, $n=20$, $\ell=26$, we obtain $\rho \approx 0.31$ via generation. This creates balanced classes while requiring models to: identify which letters matter, learn the hidden ordering, and track positions modulo the lag.

\paragraph{Parity setting}
Labels depend on the parity of key-letter occurrences. No local pattern carries information ($I(Y^{*}; X_t) = 0$ for all $t$), providing a test of pure global integration (see Theorem~\ref{prop:local-info-parity}). For $m=6$, $\lambda=7$, $n=20$, $\ell=26$, we obtain $\rho \approx 0.5$ via generation.

\subsection{Datasets}
\label{subsec:datasets}

We instantiate the framework from Section~\ref{sec:formulation} using the English alphabet ($\mathcal{A} = \{\texttt{A}, \ldots, \texttt{Z}\}$, $\ell = 26$) with sequences of length $n = 20$. For both Tricky and Parity we used $\mathcal{S} = \{\texttt{W}, \texttt{D}, \texttt{Q}, \texttt{J}, \texttt{X}, \texttt{N}\}$ with $\kappa(\texttt{W})<\kappa(\texttt{D})<\kappa(\texttt{Q})<\kappa(\texttt{J})<\kappa(\texttt{X})<\kappa(\texttt{N})$. Table~\ref{tab:datasets} summarizes the three datasets. Each dataset is designed with a specific diagnostic purpose.
...
The \textbf{tricky deterministic} dataset tests whether models can learn the sequential pattern without noise. The \textbf{tricky random} dataset introduces label noise ($\pi = 0.3$) and tests whether models plateau at $k$-gram levels or approach theoretical limits. The \textbf{parity} dataset tests pure global integration where no local pattern carries signal (Proposition~\ref{prop:local-info-parity}).
...
We evaluate Llama-3.2-1B, Llama-3.1-8B and Llama-3-70B~\citep{touvron2023llama, grattafiori2024llama3}, fine-tuned for binary classification. We also include Qwen3-4B-Think~\citep{bai2023qwen} and Qwen2.5-14B~\citep{qwen2.5, qwen2} and BERT-base (110M parameters)~\citep{devlin2019bert} with warm-start fine-tuning. These models assess whether large-scale pretraining provides advantages for sequential pattern recognition.

\paragraph{Architectures randomly initialized.}
To isolate architectural capabilities from pretrained knowledge, we train small-scale models from random initialization: Llama-style decoders (1M parameters), LSTM~\citep{hochreiter1997lstm}, Transformer encoder~\citep{vaswani2017attention}, and a hybrid BiLSTM-Transformer~\citep{graves2005bilstm}, referred to as RNN-Transformer throughout for conciseness, (100K--400K parameters each). These models disentangle architectural effects from scale effects on sequential learning.
...
The four datasets differ in their $k$-gram distributions due to distinct generation procedures. For the tricky and parity tasks, sequences were generated via brute-force sampling (uniform $1/26$ probability per letter), and then classified by the correspondent ordering criteria. For the Naive task, we followed the approach described in~\ref{paragraph:naive_setting}. This results in higher $k$-gram informativeness on the outcome across train/validation/test splits for naive compared to other datasets. In fact, in the naive setting, bigrams alone ($k=2$) carry strong signal (e.g., certain bigrams are ``forbidden'' in ordered sequences) enabling immediate classification (AUC$\sim$0.999 just with a bigram counting model). As task complexity increases, this local signal diminishes  (seeAppendix~\ref{appendix:k_gram_analysis} for detailed analysis).
...
For $\pi = 0.3$ (30\% label noise) and $\rho = 0.5$, this gives an optimal $\mathrm{AUC}^* = F_1^* = 0.70$. See Appendix~\ref{appendix:proof_theorems} for the full proof. In Table~\ref{tab:optimal_performance} are shown for each dataset the optimal performance metrics.

% PUT TABLE HERE
% as discussed in Section \ref{sec:formulation}, bounding optimal performance at $\text{AUC}^* = 0.67$ (Theorem~\ref{thm:optimal_performance}) 

\begin{table}[h]
\centering
\caption{Optimal performance achievable by a perfect classifier for each dataset.}
\label{tab:optimal_performance}
\begin{tabular}{@{}lcccc@{}}
\toprule
Dataset & AUC$^{*}$ & F1$^{*}$ & Prec$^{*}$ & Rec$^{*}$ \\
\midrule
Naive & 1.000 & 1.000 & 1.000 & 1.000\\
Tricky Det. & 1.000 &1.000 &1.000 & 1.000\\
Tricky Random & 0.670 &0.577 & 0.700 &0.491 \\
Parity &1.000 &1.000 & 1.000& 1.000\\
\bottomrule
\end{tabular}
\end{table}
...
We report AUC, F1, Precision, and Recall on held-out test sets (20\% of data). For all datasets, we compare against the theoretical optimum (Theorem~\ref{thm:optimal_performance}, indicated by symbol $^{*}$). We vary training set size from 4K to 400K samples to assess data efficiency. If (1) $\text{AUC} \approx 0.50$ that means the model learns nothing about compliance (2) $0.55 < \text{AUC} < \text{AUC}^*$: partial recovery of the sequential signal (3) $\text{AUC} \approx \text{AUC}^*$: full recovery, limited only by label noise. We adopt AUC as our primary metric since it captures a model's discriminative capacity without relying on arbitrary threshold selection.
...
Despite varying architectures from small LSTM to 70B-parameter LLMs, no model exceeded chance performance. This is not a matter of scale or training data: the task is simply too hard to any method that relies on local patterns. This is in line with the work of \citet{countingLLMs}.

\paragraph{Scale unlocks sequential learning in decoder-only models.}
A key finding is that decoder-only models \emph{can} approach the theoretical optimum on the tricky random task, but only at sufficient scale. While Llama-1B and Llama-8B plateau around AUC $\approx 0.58$ (comparable to the $k$-gram baseline), Qwen-14B approaches the theoretical ceiling of $0.67$. This suggests that autoregressive architectures are not fundamentally limited in their ability to capture global sequential dependencies, but require substantially more parameters (and data) to do so. Notably, while Qwen-14B achieves near-optimal discrimination (AUC), its F1 score is still increasing at 400K training samples (see Figure~\ref{fig:tricky_random}), indicating that the model has learned to correctly rank positive and negative examples but has not yet fully calibrated its predictions. This suggests that even larger training sets may be needed to translate discriminative capacity into optimal predictive performance. The contrast with encoder-based architectures is instructive: a custom Transformer with $\sim$100K parameters achieves the same performance as a 14B-parameter decoder. Rather than a limitation of decoder-only models, this gap indicates that next-token prediction requires greater capacity to learn classification-relevant sequential patterns. This finding nuances recent observations by \citet{nguyen2024understanding} and \citet{kimLostSequenceLarge2025b} that LLMs often reduce to local pattern matching. Our results suggest this deficiency can be overcome with sufficient scale, though the computational cost may be prohibitive for many applications.

\paragraph{Architecture and scale interact.}
The architectural comparison reveals a nuanced picture. Encoder-based models (BERT, Transformers) and recurrent architectures (LSTM, RNN-Transformer) approach the theoretical optimum with relatively few parameters. Decoder-only models can also reach this ceiling, but require substantially more capacity and more data. This reflects a fundamental mismatch: the causal attention mask forces autoregressive models to encode sequential patterns through parameters rather than bidirectional integration. 

Despite these challenges, however, our results reveal substantial potential. On tasks deliberately designed to eliminate semantic content, encoder-based transformers and sufficiently scaled decoders still approach theoretical optima, suggesting these models develop internal representations capable of capturing sequential structure even without semantic scaffolding. The scaling trends we observe point toward a promising direction: leveraging the rich representations within LLMs for sequential tasks where not just order but also meaning \emph{does} matter, combining their pattern-learning capabilities with their vast pretrained knowledge.

\paragraph{Limitations and future directions.} Our study has several limitations that suggest directions for future work. We tested sequences of fixed length ($n=20$) and alphabet size ($\ell=26$) and $\lambda=7$, when used; examining how performance scales with sequence length could reveal whether the encoder-decoder gap widens or narrows for longer contexts. While we tested models up to 14B parameters (Llama-70B only on parity), larger models might exhibit additional scaling effects. Our synthetic tasks, while controlled, may not capture all aspects of real-world sequential data extending our framework to clinical trajectories or other temporal data could validate whether these patterns transfer to practice. The efficiency gap between encoders and decoders may also narrow in settings where semantic understanding complements sequential structure. Finally, we focused on classification; generative tasks might engage different capabilities. Other promising directions include mechanistic interpretability analyses to clarify what features models actually learn, and architectural modifications such as auxiliary objectives or structural inductive biases that explicitly target global integration.
...
% Key information - moved down more
\node at (7.5,-3) {Key: $\mathcal{S} = \{\texttt{D}, \texttt{Q}, \texttt{J}, \texttt{W}, \texttt{X}, \texttt{N}\}$, 
with $\kappa(\texttt{W}) < \kappa(\texttt{D}) < \kappa(\texttt{Q}) < \kappa(\texttt{J}) < \kappa(\texttt{X}) < \kappa(\texttt{N})$, $\lambda=5$};
...
% Key information
\node at (7.5,-1.5) {Key: $\mathcal{S} = \{\texttt{D}, \texttt{Q}, \texttt{J}, \texttt{W}, \texttt{X}, \texttt{N}\}$};
...
\end{tikzpicture}
\caption{Sequence classification example using the \textit{parity} method. Key letters $\mathcal{S} = \{\texttt{D}, \texttt{Q}, \texttt{J}, \texttt{W}, \texttt{X}, \texttt{N}\}$ are extracted from the full sequence (non-key letters in gray).
...
\begin{table}[h!]
\centering
\caption{k-gram baseline predictive performance across datasets. Results shown for optimized classification thresholds. \textit{Tricky Deterministic}: Deterministic sequence with statistical shortcuts. \textit{Tricky Random}: Same structure with added noise ($\pi=0.7$). \textit{Parity}: Parity task requiring full sequence context.}
...
In the naive setting, compliance requires $\kappa(X_1) \leq \kappa(X_2) \leq \cdots \leq \kappa(X_n)$. The number of such non-decreasing sequences equals the number of multisets of size $n$ from $\ell$ elements, which is $\binom{n+\ell-1}{n}$ by the so called stars and bars formula \citep{Stars_Bars}. Dividing by the total $\ell^n$ sequences gives the result.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:optimal_performance}]
\label{proof:optimal_performance}
The \textit{observed} outcome distribution conditional on compliance status $Y^{*} \in \{0,1\}$ is $f(Y\mid Y^{*}=1) \sim \mathrm{Bernoulli}(1-\pi)$ and $f(Y \mid Y^{*}=0) \sim \mathrm{Bernoulli}(\pi)$. 

\emph{(a)} Given $Y^{*}$, the optimal prediction is $\hat{Y} = Y^{*}$ (predict 1 if compliant, 0 otherwise), achieving accuracy $\mathbb{P}(\text{correct} \mid Y^{*}=1) \cdot \rho + \mathbb{P}(\text{correct} \mid Y^{*}=0) \cdot (1-\rho) = (1-\pi)\rho + (1-\pi)(1-\rho) = 1-\pi$, where $\rho$ denotes the compliance probability (i.e., $\mathbb{P}(Y^{*}=1)=\rho$).

\emph{(b)} The marginal $\mathbb{P}(Y=1) = \rho\pi + (1-\rho)(1-\pi)$. For $\rho = 0.5$, this equals $0.5$, so both classes are equally likely and random guessing achieves $50\%$.
...
\begin{align}
\mathbb{P}(Y = 1, Y^* = 1) &= (1-\pi)\rho \\
\mathbb{P}(Y = 1, Y^* = 0) &= \pi(1-\rho) \\
\mathbb{P}(Y = 0, Y^* = 1) &= \pi\rho \\
\mathbb{P}(Y = 0, Y^* = 0) &= (1-\pi)(1-\rho)
\end{align}

Marginal Distributions:

\begin{align}
\mathbb{P}(Y = 1) &= \rho(1-\pi) + (1-\rho)\pi \\
\mathbb{P}(Y = 0) &= \rho \pi + (1-\rho)(1-\pi)
\end{align}
...
\begin{align}
    A &= \mathbb{P}(s = 1 \mid Y = 1) = \frac{(1-\pi)\rho}{\rho(1-\pi) + (1-\rho)\pi} \\
    B &= \mathbb{P}(s = 0 \mid Y = 1) = \frac{\pi(1-\rho)}{\rho(1-\pi) + (1-\rho)\pi} \\
    C &= \mathbb{P}(s = 1 \mid Y = 0) = \frac{\pi\rho}{\rho \pi + (1-\rho)(1-\pi)} \\
    D &= \mathbb{P}(s = 0 \mid Y = 0) = \frac{(1-\pi)(1-\rho)}{\rho \pi + (1-\rho)(1-\pi)}
\end{align}
...
\[
|\mathcal{C}_n| = \binom{n + \ell - 1}{\ell - 1}.
\]

Fix the position $t=1$.  
Let $a_j \in \mathcal{A}$ be a symbol with rank $j = \kappa(a_j)$.  
If $X_1 = a_j$, then the remaining $n-1$ symbols must form a non-decreasing sequence over the alphabet $\{a_j, a_{j+1}, \dots, a_\ell\}$.  
The number of such sequences is
\[
|\mathcal{C}_{n,1,j}| = \binom{n - 1 + \ell - j}{\ell - j}.
\]

The conditional probability is therefore
\[
\mathbb{P}(X_1 = a_j \mid Y^{*} = 1)
= \frac{|\mathcal{C}_{n,1,j}|}{|\mathcal{C}_n|}
= \frac{\binom{n - 1 + \ell - j}{\ell - j}}{\binom{n + \ell - 1}{\ell - 1}}.
\]
This quantity depends explicitly on $j$. In contrast, under the unconditional distribution,
\[
\mathbb{P}(X_1 = a_j) = \frac{1}{\ell} \qquad \forall j.
\]
...
\subsection{Proof of Theorem~\ref{prop:local-info-parity}}
\label{app:proof_prop_local_info}

To prove Proposition \ref{prop:local-info-parity}, we first introduce an auxiliary variable representing the standard parity of the sequence with respect to the subset $\mathcal{S}$, and establish its local information properties.

\begin{lemma}[Auxiliary Parity Variable]
\label{lemma:standard_parity}
Define the variable $T := \left(\sum_{t=1}^n \mathbf{1}_{\{X_t \in \mathcal{S}\}}\right) \mod 2$. 
Let $p = |\mathcal{S}|/|\mathcal{A}|$. Then:
\begin{itemize}
...
\begin{proof}[Proof of Lemma \ref{lemma:standard_parity}]
    Fix $t$ and write $S := \sum_{i=1}^n \mathbf{1}_{\{X_i \in \mathcal{S}\}} = \mathbf{1}_{\{X_t \in \mathcal{S}\}} + S_{-t}$, where $S_{-t} := \sum_{i \neq t} \mathbf{1}_{\{X_i \in \mathcal{S}\}}$. 
...
Recall $Y = (\sum_{s \in \mathcal{S}} \mathbf{1}_{\{N_s \text{ even}\}}) \mod 2$. Using the identity $\mathbf{1}_{\{n \text{ even}\}} = 1 \oplus (n \pmod 2)$ where $\oplus$ is addition modulo 2:
\begin{align*}
    Y &= \bigoplus_{s \in \mathcal{S}} (1 \oplus (N_s \pmod 2)) \\
      &= \left(\bigoplus_{s \in \mathcal{S}} 1\right) \oplus \left(\bigoplus_{s \in \mathcal{S}} (N_s \pmod 2)\right).
\end{align*}
The first term is $|\mathcal{S}| \pmod 2$. The second term is $(\sum_{s \in \mathcal{S}} N_s) \pmod 2$, which is exactly $\sum_{t=1}^n \mathbf{1}_{\{X_t \in \mathcal{S}\}} \pmod 2 = T$.
Thus:
\[
    Y = (|\mathcal{S}| \pmod 2) \oplus T.
\]
For a fixed set $\mathcal{S}$, the term $(|\mathcal{S}| \pmod 2)$ is a constant. The mapping $T \mapsto Y$ is therefore a bijection (identity or bit-flip). Since mutual information is invariant under bijective transformations:
\[
    I(Y; X_t) = I(T; X_t).
\]
