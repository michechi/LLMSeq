...
def main(args):
    ...
    for model_name in models_to_train:
        ...
        if args.no_search:
            ...
            logger.info(f"Using optimal configuration (no search)")
            best_config = OPTIMAL_CONFIGS[model_name]
            best_lr = OPTIMAL_LR[model_name]
            search_results = None
        else:
...
Examples:
  ...
  python rnn_baseline_trainer.py \\
    ...
    parser.add_argument('--use_optimal', action='store_true',
                       help='Skip hyperparameter search, use optimal configs')
