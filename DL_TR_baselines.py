...
def main(args):
    ...
    for model_name in models_to_train:
        ...
        # Hyperparameter search or use default/optimal
        if args.optimal:
            logger.info(f"Using OPTIMAL configuration")
            best_config = OPTIMAL_CONFIGS[model_name].copy()
            best_lr = OPTIMAL_LR.get(model_name, OPTIMAL_LR['LSTM'])
            search_results = None
        elif args.no_search:
            logger.info(f"Using default configuration (no search)")
            best_config = DEFAULT_CONFIGS[model_name].copy()
...
Examples:
  ...
  # Use OPTIMAL (tuned) configurations
  python DL_TR_baselines.py \\
      --path_csv /path/to/data/ \\
      --number_to_use 1 \\
      --optimal

  # Specific models only with optimal configs
  python DL_TR_baselines.py \\
      --path_csv /path/to/data/ \\
      --number_to_use 1 \\
      --models Transformer,RNNTransformer \\
      --optimal
        """
    )
    ...
    parser.add_argument('--path_csv', type=str, 
                       default='/fp/homes01/u01/ec-michelec/MIMICIV/data/simulation/',
    ...
    parser.add_argument('--optimal', action='store_true',
                       help='Use optimal (tuned) configurations instead of default/search')
